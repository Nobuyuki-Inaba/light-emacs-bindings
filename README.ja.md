...
- **コピー・カット・ペースト(`Ctrl+C` / `Ctrl+X` / `Ctrl+V`)は絶対に壊しません。** `Ctrl+X` はEmacsの `C-x` プレフィックスも兼ねますが、約1秒以内に続きの入力が無ければ自動的にネイティブなCutにフォールバックします([Ctrl+Xの仕組み](#ctrlxの仕組み)参照)。
...
1. `Ctrl+X` を押すと約1秒間の待機ウィンドウが始まります。
